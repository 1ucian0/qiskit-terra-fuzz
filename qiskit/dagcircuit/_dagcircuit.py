...
class DAGCircuit:
    ...
    def _check_wires_list(self, wires, node):
        ...
        Args:
            wires (list[register, index]): gives an order for (qu)bits
                in the input_circuit that is replacing the node.
    ...
    def substitute_node_with_dag(self, node, input_dag, wires=None):
        ...
        # the dag must be ammended if the node is used in a
        # conditional context. delete the op nodes of input_dag, 
        # and replay them with the condition.
        if condition:
            ...
            for n in to_replay:
                input_dag.apply_operation_back(n["op"], n["qargs"], n["cargs"], condition=condition)
